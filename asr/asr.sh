    ...
    if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
        ...
        for dset in "${train_set}" "${valid_set}"; do
            ...
            if [ "${_feats_type}" = raw ]; then
                ...
                <"${data_feats}/org/${dset}/utt2spkvec.scp" \
                    utils/filter_scp.pl "${data_feats}/${dset}/utt2num_samples"  \
		    >"${data_feats}/${dset}/utt2spkvec.scp" 
            else
        ...
        #Shell script to enable data correction and speaker vector generation
        local/spkdatavec.sh
    fi
...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 9 ] && [ ${stop_stage} -ge 9 ]; then
        ...
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.asr_train \
                ...
                --train_data_path_and_name_and_type "${_asr_train_dir}/utt2spkvec.scp,speaker_id,npy" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/${_scp},speech,${_type}" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/text,text,text" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/utt2spkvec.scp,speaker_id,npy" \
                --train_shape_file "${_logdir}/train.JOB.scp" \
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        ...
        if [ "${num_splits_asr}" -gt 1 ]; then
            ...
            _opts+="--train_data_path_and_name_and_type ${_split_dir}/utt2spkvec.scp,speaker_id,npy "
            ...
            _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/utt2spkvec.scp,speaker_id,npy "
        ...
        for dset in ${test_sets}; do
            ...
            ${_cmd} --gpu "${_ngpu}" JOB=1:"${_nj}" "${_logdir}"/asr_inference.JOB.log \
                ${python} -m espnet2.bin.asr_inference \
                    ...
                    --data_path_and_name_and_type "${_data}/utt2spkvec.scp,speaker_id,npy" \
