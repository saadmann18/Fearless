...
def main(_run, exp_dir, storage_dir, database_json, ckpt_name, subset, batch_size, device):
    ...
    with torch.no_grad():
        ...
        #thr,met = sed.get_optimal_thresholds(targets,scores,metric='f1')       
    ...
    #           
    #           
    #           pred_max = np.argmin(new_thr, axis=-1) #max and relative dist measure
    #           accuracy = (pred_max == target).mean()            
    #           Accuracy.append(accuracy)
    #           
    #       print(np.mean(Accuracy))           
        ...
        dump_json(metric, path = '/net/home/dheerajpr/my_project/fearless/fearless/sid/eval/result_conv3.json', indent = 4, sort_keys = False)
